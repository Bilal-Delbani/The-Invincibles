...
function GetMoney($crdt){
    ...
    $query = "SELECT `BALANCE` FROM `banking_system` WHERE (`ACCOUNT_NUMBER`='".$crdt."')";

    $stmt=$obj->query($query);
    while($record=$stmt->fetch()){
      $answer= $record["BALANCE"];
...
function updateMoney($crdt,$money){
    ...
    $query = "UPDATE `banking_system` SET `BALANCE`=MONEY-$money WHERE (`ACCOUNT_NUMBER`='".$crdt."')";
