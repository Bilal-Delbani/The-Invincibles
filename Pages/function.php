...
{
    $query = "SELECT * FROM theplayers_table WHERE POSITION = :position ORDER BY ID DESC";
    $statement = $connect->prepare($query);
    ...
    foreach ($result as $row) {
        ...
        $output .= '<td>';
        $output .= '<button type="button" onclick="fetch_data(' . $row["ID"] . ')" class="btn btn-warning btn-sm" style="margin-right:5px;">Edit</button>';
        $output .= '<button type="button" onclick="delete_data(' . $row["ID"] . ')" class="btn btn-danger btn-sm">Delete</button>';
        $output .= '</td>';
        $output .= '</tr>';
    }
    return $output;
...
function feedback($connect)
{
    $query = "SELECT * FROM feedback ORDER BY FAN_ID DESC";
    $statement = $connect->prepare($query);
    $statement->execute();
    $result = $statement->fetchAll(PDO::FETCH_ASSOC);
    $output = '';
    foreach ($result as $row) {
        $output .= '<tr>';
        $output .= '<td>' . $row["FEEDB"] . '</td>';
        $output .= '<td>';
        $output .= '<button type="button" onclick="delete_data(' . $row["FAN_ID"] . ')" class="btn btn-danger btn-sm">Delete</button>';
        $output .= '</td>';
        $output .= '</tr>';
    }
    return $output;
